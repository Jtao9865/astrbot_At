...
class LLMAtToolPlugin(Star):
    ...
    async def get_group_members(
        self, context: Context, keyword: str = ""
    ) -> str:
        """
        ...
        # 兼容处理：从 ContextWrapper 中提取原始事件
        event = context.get_event() if hasattr(context, 'get_event') else context
        if not hasattr(event, 'get_group_id'):
            return json.dumps(
                {"status": "error", "message": "当前上下文无法获取群组信息。"},
                ensure_ascii=False,
            )
